...
fn main() -> wasmtime::Result<()> {
    ...
    if args.len() < 2 {
        eprintln!("Usage: japl-serve <file.wasm> [--port PORT]");
        std::process::exit(1);
    }
    ...
    while i < args.len() {
        if args[i] == "--port" && i + 1 < args.len() {
            port = args[i + 1].parse().map_err(|e| Error::msg(format!("Invalid port: {}", e)))?;
            i += 2;
        } else {
            i += 1;
        }
    ...
    let server = tiny_http::Server::http(format!("0.0.0.0:{}", port))
        .map_err(|e| Error::msg(format!("Failed to bind HTTP server: {}", e)))?;
...
/// Write a string into WASM memory in JAPL format: [4-byte LE length][bytes]
/// Returns the pointer (i32) to the start of the JAPL string.
#[allow(dead_code)]
fn write_japl_string(store: &mut Store<()>, instance: &Instance, s: &str) -> wasmtime::Result<i32> {
    ...
    if heap_ptr + total > mem_size {
        let pages_needed = ((heap_ptr + total - mem_size) + 65535) / 65536;
        memory.grow(&mut *store, pages_needed as u64)?;
    }

    let data = memory.data_mut(&mut *store);
    // Write length (LE u32)
    data[heap_ptr..heap_ptr + 4].copy_from_slice(&(bytes.len() as u32).to_le_bytes());
    // Write string bytes
    data[heap_ptr + 4..heap_ptr + 4 + bytes.len()].copy_from_slice(bytes);
...
/// Read a string from WASM memory given a raw (ptr, len) pair — NOT JAPL format,
/// just raw bytes at ptr with the given length.
...
fn handle_wasm_request(
    ...
    // Write method, path, body as raw byte regions (NOT JAPL strings).
    // __handle_http takes (ptr, len) pairs and internally packs them into JAPL strings.
    let memory = instance.get_memory(&mut *store, "memory")
        .ok_or_else(|| Error::msg("No memory export"))?;
    let heap_global = instance.get_global(&mut *store, "heap_ptr")
        .ok_or_else(|| Error::msg("No heap_ptr export"))?;
    ...
    // Allocate raw byte regions for each string
    ...
    // __handle_http returns (data_ptr, len) where data_ptr points past the 4-byte length prefix
    read_string_from_wasm(store, instance, resp_ptr, resp_len)
}

/// Register all JAPL host functions. For japl-serve we provide simplified stubs:
/// - String functions are fully implemented (needed for HTTP handling)
/// - Process/TCP/etc functions are no-ops or return errors (not needed for HTTP serving)
fn register_host_functions(linker: &mut Linker<()>) -> wasmtime::Result<()> {
    ...
    // japl.char_at(str_ptr: i32, index: i32) -> i32
    ...
    // japl.substring(str_ptr: i32, start: i32, end: i32) -> i32
    ...
    // japl.string_index_of(hay_ptr: i32, needle_ptr: i32) -> i32
    ...
    // japl.from_char_code(code: i32) -> i32
    ...
    // japl.string_eq(a_ptr: i32, b_ptr: i32) -> i32
    ...
    // =========================================================================
    // Stubs for functions that WASM might import but aren't needed for HTTP
    // =========================================================================
