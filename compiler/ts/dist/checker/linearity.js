...
export class LinearityChecker {
    ...
    checkFnDecl(decl) {
        ...
        for (const [name, info] of useCounts) {
            if (info.count > 1) {
                this.errors.push(new TypeError(`Owned value '${name}' used more than once (${info.count} uses). Owned values must be used exactly once`, info.spans[1] ?? decl.span, ['Consider cloning the value or restructuring to use it only once']));
            }
            else if (info.count === 0) {
                this.errors.push(new TypeError(`Owned value '${name}' is never used. Owned values must be used exactly once`, decl.span, ['Use the value or explicitly drop it']));
            }
        }
    }
    isOwnedType(type) {
    ...
    countVarUses(expr, tracked, counts) {
        switch (expr.kind) {
            case 'var':
            ...
            case 'app':
                ...
                for (const arg of expr.args)
                    this.countVarUses(arg, tracked, counts);
                break;
            case 'lambda':
            ...
            case 'if':
                ...
                if (expr.else)
                    this.countVarUses(expr.else, tracked, counts);
                break;
            case 'match':
            ...
            case 'block':
                for (const e of expr.exprs)
                    this.countVarUses(e, tracked, counts);
                break;
            case 'binop':
            ...
            case 'pipe':
                ...
                break;
            case 'record':
                for (const [, val] of expr.fields)
                    this.countVarUses(val, tracked, counts);
                break;
            case 'field_access':
...
//# sourceMappingURL=linearity.js.map