...
export class TypeChecker {
    unifier = new UnificationEngine();
    env = new TypeEnv();
    effectChecker = new EffectChecker();
    errors = [];
    exprTypes = new Map();
    constructor() {
    ...
    inferTypeDecl(decl) {
        ...
        const typeParamVars = new Map();
        for (const tp of decl.typeParams) {
            typeParamVars.set(tp, freshVar());
        }
        // Build the result type
        const resultType = decl.typeParams.length === 0
    ...
    inferRecordTypeDecl(decl) {
        ...
        const recordType = { kind: "record", fields };
        this.env.defineType(decl.name, {
            name: decl.name,
            typeParams: decl.typeParams,
            variants: [],
    ...
    inferForeignDecl(decl) {
        const paramTypes = decl.params.map(p => this.lowerTypeExpr(p.type));
    ...
    inferConstructor(expr) {
        ...
        const freshMapping = new Map();
        const instantiateCtorType = (t) => this.instantiateType(t, freshMapping);
        ...
        if (expr.args.length !== expectedArgTypes.length) {
            this.errors.push(new TypeError(`Constructor ${expr.name} expects ${expectedArgTypes.length} argument${expectedArgTypes.length === 1 ? '' : 's'} but got ${expr.args.length}`, expr.span));
    ...
    inferApp(expr) {
        ...
        if (resolvedFn.kind === "fn") {
            if (expr.args.length !== resolvedFn.params.length) {
                this.errors.push(new TypeError(`Function expects ${resolvedFn.params.length} argument${resolvedFn.params.length === 1 ? '' : 's'} but got ${expr.args.length}`, expr.span));
        ...
        const argResults = expr.args.map(a => this.inferExpr(a));
        ...
        switch (pat.kind) {
            ...
            case "pconstructor": {
                ...
                if (pat.args.length !== fieldTypes.length) {
                    this.errors.push(new TypeError(`Constructor ${pat.name} expects ${fieldTypes.length} argument${fieldTypes.length === 1 ? '' : 's'} but got ${pat.args.length}`, pat.span));
                    return;
                }
            ...
            case "plist": {
                ...
                const listType = { kind: "list", element: elemType };
                try {
            ...
            case "ptuple": {
                ...
                const tupleType = { kind: "tuple", elements: elemTypes };
                try {
    ...
    inferPipe(expr) {
        ...
        if (expr.right.kind === "app") {
            ...
            const argResults = expr.right.args.map(a => this.inferExpr(a));
    ...
    inferFieldAccess(expr) {
        ...
        if (resolved.kind === "record") {
            ...
            this.errors.push(new TypeError(`Record has no field '${expr.field}'`, expr.span));
        ...
        if (resolved.kind === "var") {
            ...
            const expectedRec = { kind: "record", fields: recFields, row: rowId };
            try {
        ...
        this.errors.push(new TypeError(`Cannot access field '${expr.field}' on type ${typeToString(resolved)}`, expr.span));
        return [freshVar(), recEff];
    }
    inferRecordUpdate(expr) {
    ...
    inferSpawn(expr) {
        ...
        const pidType = { kind: "pid", msg: msgType };
        const processEffect = { effects: new Set(["process"]), open: false };
    ...
    inferSend(expr) {
        ...
        const expectedPid = { kind: "pid", msg: msgType };
        try {
        ...
        const effects = this.mergeEffects(this.mergeEffects(targetEff, msgEff), { effects: new Set(["process"]), open: false });
        return [UNIT, effects];
    }
    inferReceive(expr) {
        ...
        const msgType = freshVar();
        const resultType = freshVar();
        let effects = { effects: new Set(["process"]), open: false };
        for (const arm of expr.arms) {
            this.env.pushScope();
    ...
    inferTry(expr) {
        ...
        const expectedResult = { kind: "result", ok: okType, err: errType };
        try {
        ...
        const effects = this.mergeEffects(innerEff, { effects: new Set(["fail"]), open: false });
    ...
    lowerTypeExprWithParams(te, typeParams) {
        switch (te.kind) {
            case "tnamed": {
