...
export function lowerModule(module) {
    const decls = [];
    ...
    switch (decl.kind) {
        ...
        case "record_type":
            return {
                kind: "record_type",
                name: decl.name,
                fields: decl.fields.map(f => [f.name, lowerTypeExprToString(f.type)]),
            };
        case "test":
            return {
                kind: "test",
...
function lowerTypeExprToString(type) {
    switch (type.kind) {
        case "tnamed":
...
function lowerExpr(expr) {
    switch (expr.kind) {
        ...
        case "record":
            return {
                kind: "record",
                fields: expr.fields.map(([name, val]) => [name, lowerExpr(val)]),
            };
        case "field_access":
            return {
                kind: "field_access",
        ...
        case "record_update":
            return {
                ...
                updates: expr.fields.map(([name, val]) => [name, lowerExpr(val)]),
            };
        case "list":
            return {
                kind: "list",
