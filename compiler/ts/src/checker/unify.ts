import { Type, typeToString } from './types.js';
...
export class UnificationEngine {
  ...
  unify(a: Type, b: Type, span: Span): void {
    ...
    if (ra.kind === "var") {
      if (rb.kind === "var" && ra.id === rb.id) return;
      if (this.occursCheck(ra.id, rb)) {
        throw new TypeError(
          `Infinite type: ${typeToString(ra)} occurs in ${typeToString(rb)}`,
          span,
        );
      }
    ...
    if (rb.kind === "var") {
      if (this.occursCheck(rb.id, ra)) {
        throw new TypeError(
          `Infinite type: ${typeToString(rb)} occurs in ${typeToString(ra)}`,
          span,
        );
      }
    ...
    if (ra.kind === "never" || rb.kind === "never") return;

    // Same structural kind: recurse
    if (ra.kind !== rb.kind) {
      throw new TypeError(
        `Expected ${typeToString(ra)} but got ${typeToString(rb)}`,
        span,
      );
    }
    ...
    switch (ra.kind) {
      case "fn": {
        const rbFn = rb as typeof ra;
        if (ra.params.length !== rbFn.params.length) {
          throw new TypeError(
            `Function expects ${ra.params.length} arguments but got ${rbFn.params.length}`,
            span,
          );
        }
      ...
      case "named": {
        ...
        if (ra.args.length !== rbNamed.args.length) {
          throw new TypeError(
            `Type ${ra.name} expects ${ra.args.length} type arguments but got ${rbNamed.args.length}`,
            span,
          );
        }
      ...
      case "record": {
        const rbRec = rb as typeof ra;
        this.unifyRecord(ra, rbRec, span);
        break;
      }
      case "tuple": {
        const rbTup = rb as typeof ra;
        if (ra.elements.length !== rbTup.elements.length) {
          throw new TypeError(
            `Tuple has ${ra.elements.length} elements but expected ${rbTup.elements.length}`,
            span,
          );
        }
  ...
  private unifyRecord(
    a: Extract<Type, { kind: "record" }>,
    b: Extract<Type, { kind: "record" }>,
    span: Span,
  ): void {
    // Unify shared fields
    ...
    for (const [key] of b.fields) {
      if (!a.fields.has(key) && a.row === undefined) {
        throw new TypeError(
          `Record has unexpected field '${key}'`,
          span,
        );
      }
    }
    ...
    if (a.row !== undefined && b.row !== undefined) {
      ...
      const aExtras = new Map<string, Type>();
      for (const [k, v] of a.fields) {
        if (!b.fields.has(k)) aExtras.set(k, v);
      }
      const bExtras = new Map<string, Type>();
      for (const [k, v] of b.fields) {
        if (!a.fields.has(k)) bExtras.set(k, v);
      }
  ...
  private unifyEffects(
    a: { effects: Set<string>; open: boolean },
    b: { effects: Set<string>; open: boolean },
    _span: Span,
  ): void {
  ...
  private occursCheck(id: number, t: Type): boolean {
    const resolved = this.resolve(t);
    switch (resolved.kind) {
      case "var":
      ...
      case "record":
        for (const v of resolved.fields.values()) {
          if (this.occursCheck(id, v)) return true;
        }
