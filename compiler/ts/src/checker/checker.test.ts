...
import { TypeChecker, TypedModule } from './infer.js';
import { Type, Effect, resetVarCounter, typeToString, INT, FLOAT, BYTE, STRING, BOOL, UNIT } from './types.js';
...
function ident(name: string): Token { return tok(TokenKind.Ident, name); }
function upper(name: string): Token { return tok(TokenKind.UpperIdent, name); }
function int(value: number): Token { return tok(TokenKind.Int, String(value)); }
function float(value: number): Token { return tok(TokenKind.Float, String(value)); }
function str(value: string): Token { return tok(TokenKind.String, value); }
function eof(): Token { return tok(TokenKind.EOF); }
...
function checkModule(tokens: Token[]): { checker: TypeChecker; result: TypedModule } {
...
function inferExprType(tokens: Token[]): { type: Type; checker: TypeChecker; errors: TypeError[] } {
...
function checkOk(tokens: Token[]): { checker: TypeChecker; result: TypedModule } {
...
function checkErr(tokens: Token[]): { checker: TypeChecker; result: TypedModule } {
  const r = checkModule(tokens);
  expect(r.result.errors.length).toBeGreaterThan(0);
  return r;
}

function checkSource(source: string): { checker: TypeChecker; result: TypedModule } {
...
describe("UnificationEngine", () => {
  ...
  it("detects infinite type via occurs check", () => {
    const engine = new UnificationEngine();
    const v: Type = { kind: "var", id: 0 };
    const listV: Type = { kind: "list", element: v };
  ...
  it("deeply resolves chained variables", () => {
    const engine = new UnificationEngine();
    const v1: Type = { kind: "var", id: 0 };
    const v2: Type = { kind: "var", id: 1 };
  ...
  it("unifies list types", () => {
    const engine = new UnificationEngine();
    const list1: Type = { kind: "list", element: INT };
    const list2: Type = { kind: "list", element: INT };
  ...
  it("fails to unify List[Int] with List[String]", () => {
    const engine = new UnificationEngine();
    const list1: Type = { kind: "list", element: INT };
    const list2: Type = { kind: "list", element: STRING };
...
describe("EffectChecker", () => {
  ...
  it("reports error when pure function has IO effect", () => {
    ...
    const declared = { effects: new Set<Effect>(), open: false }; // pure
    const actual = { effects: new Set<Effect>(["io"]), open: false };
