import { EffectRow, effectRowToString } from './types.js';
...
export class EffectChecker {
   ...
   */
  checkPurity(
    declaredEffects: EffectRow | undefined,
    inferredEffects: EffectRow,
    span: Span,
  ): TypeError | null {
    if (!declaredEffects) return null; // no annotation: effects are inferred
    ...
    if (declaredEffects.effects.size === 0 && !declaredEffects.open) {
      if (inferredEffects.effects.size > 0) {
        ...
        return new TypeError(
          `Function is declared pure but has effects: ${effStr}`,
          span,
          ["Remove the effectful operations or add an effect annotation"],
        );
      }
      return null;
    }
  ...
  checkSubsumption(
    ...
    if (declared.open) return null; // open row allows anything

    for (const eff of actual.effects) {
      if (!declared.effects.has(eff) && !this.isSubsumedByDeclared(eff, declared)) {
        return new TypeError(
          `Cannot use ${eff.toUpperCase()} operation in function declared with effects ${effectRowToString(declared) || "Pure"}`,
          span,
          [`Add '${eff}' to the function's effect annotation`],
        );
      }
    }
    return null;
   ...
   */
  private isSubsumedByDeclared(eff: string, declared: EffectRow): boolean {
    // pure is always subsumed
    if (eff === "pure") return true;
    // async is subsumed by process
    if (eff === "async" && declared.effects.has("process")) return true;
    return false;
  }
}
