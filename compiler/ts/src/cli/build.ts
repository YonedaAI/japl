...
import { MultiFileCompiler, CompileResult } from '../modules/compiler.js';
...
export function buildToString(source: string, targetOrOpts: Target | BuildOptions = 'ts'): string {
  const opts: BuildOptions = typeof targetOrOpts === 'string'
    ? { target: targetOrOpts }
    : targetOrOpts;
 ...
 */
export function buildMultiFile(inputPath: string, extraSearchPaths: string[] = []): CompileResult {
 ...
 */
export function buildMultiFileTo(inputPath: string, outDir: string, extraSearchPaths: string[] = []): string {
 ...
 */
function fileHasImports(inputPath: string): boolean {
