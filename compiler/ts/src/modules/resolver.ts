...
export interface ResolvedModule {
  ...
  path: string;
  ast: AST.Module;
  exports: Map<string, AST.Decl>;  // only pub declarations
  allDecls: Map<string, AST.Decl>; // all declarations (for error messages)
}
  ...
  resolve(moduleName: string, fromFile: string): ResolvedModule {
    ...
    if (!resolvedPath) {
      throw new ModuleError(
        `Cannot find module "${moduleName}". Searched:\n` +
    ...
    if (parseErrors.length > 0) {
      throw new ModuleError(
        `Parse errors in module "${moduleName}" (${resolvedPath}):\n` +
        parseErrors.map(e => `  ${e.message}`).join('\n')
      );
    }

    // Collect exports (pub) and all declarations
    const exports = new Map<string, AST.Decl>();
    const allDecls = new Map<string, AST.Decl>();
