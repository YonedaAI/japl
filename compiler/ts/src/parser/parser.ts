...
// Binding powers for Pratt parsing (left, right)
// Higher number = tighter binding
type BindingPower = [number, number];

function infixBp(kind: TokenKind): BindingPower | null {
  switch (kind) {
...
function opString(kind: TokenKind): string {
  switch (kind) {
...
export class Parser {
  ...
  constructor(tokens: Token[]) {
    ...
    this.tokens = tokens.filter(
      (t) => t.kind !== TokenKind.Comment && t.kind !== TokenKind.Newline,
    );
    this.pos = 0;
    this.errors = [];
  }
  ...
  private parseFnDecl(pub: boolean): AST.Decl {
    ...
    let effects: AST.EffectExpr | undefined;
  ...
  private parseTypeDecl(): AST.Decl {
    const startPos = this.pos;
  ...
  private parseTraitDecl(): AST.Decl {
    const startPos = this.pos;
  ...
  private parseImplDecl(): AST.Decl {
    const startPos = this.pos;
  ...
  private parseModuleDecl(): AST.Decl {
    const startPos = this.pos;
  ...
  private parseImportDecl(): AST.Decl {
    const startPos = this.pos;
  ...
  private parseTestDecl(): AST.Decl {
    const startPos = this.pos;
  ...
  private parseSupervisorDecl(): AST.Decl {
    const startPos = this.pos;
  ...
  parseExpr(minBp: number = 0): AST.Expr {
  ...
  private parsePrefixExpr(): AST.Expr {
    const tok = this.current();
  ...
  private parseParenExpr(): AST.Expr {
    const startPos = this.pos;
    this.expect(TokenKind.LParen);
    // Unit: ()
  ...
  private parseMatchExpr(): AST.Expr {
    const startPos = this.pos;
  ...
  private parseLambdaExpr(): AST.Expr {
    const startPos = this.pos;
    this.expect(TokenKind.Fn);
    this.expect(TokenKind.LParen);
    const params = this.parseParamList();
  ...
  private parseBlockBody(): AST.Expr {
    const startPos = this.pos;
  ...
  private parseReceiveExpr(): AST.Expr {
    const startPos = this.pos;
  ...
  private parseReturnExpr(): AST.Expr {
    const startPos = this.pos;
  ...
  private parseRecordOrBlock(): AST.Expr {
    const startPos = this.pos;
    this.advance(); // consume {
    ...
    const exprs: AST.Expr[] = [firstExpr];
    while (!this.check(TokenKind.RBrace) && !this.atEnd()) {
  ...
  private parseRecordLiteral(startPos: number): AST.Expr {
    const fields: [string, AST.Expr][] = [];
    do {
      const fName = this.expect(TokenKind.Ident).value;
      this.expect(TokenKind.Colon);
  ...
  private parseListExpr(): AST.Expr {
    const startPos = this.pos;
  ...
  private parseRecordPattern(): AST.Pattern {
    const startPos = this.pos;
    this.expect(TokenKind.LBrace);
  ...
  private parseListPattern(): AST.Pattern {
    const startPos = this.pos;
    this.expect(TokenKind.LBracket);
  ...
  private parseTuplePattern(): AST.Pattern {
    const startPos = this.pos;
    this.expect(TokenKind.LParen);
    // Could be () for unit
    if (this.match(TokenKind.RParen)) {
  ...
  private parseFnTypeExpr(): AST.TypeExpr {
    const startPos = this.pos;
    this.expect(TokenKind.Fn);
    this.expect(TokenKind.LParen);
  ...
  private parseRecordTypeExpr(): AST.TypeExpr {
    const startPos = this.pos;
    this.expect(TokenKind.LBrace);
    const fields: [string, AST.TypeExpr][] = [];
  ...
  private parseEffectExpr(): AST.EffectExpr {
    const startPos = this.pos;
  ...
  private current(): Token {
    if (this.pos >= this.tokens.length) {
      // Return EOF token
      const lastSpan: Span = this.tokens.length > 0
  ...
  private expect(kind: TokenKind): Token {
    ...
    if (tok.kind !== kind) {
      throw this.error(
        `Expected ${tokenKindName(kind)}, got ${tokenKindName(tok.kind)} '${tok.value}'`,
      );
    }
    return this.advance();
  }
  ...
  private spanFrom(startPos: number): Span {
  ...
  private spanFromExpr(expr: AST.Expr, endPos: number): Span {
