...
function applyTCO(module: IR.IrModule): IR.IrModule {
  return {
...
function hasTailCallTo(expr: IR.IrExpr, fnName: string): boolean {
  switch (expr.kind) {
    case "app":
...
function rewriteTailCalls(expr: IR.IrExpr, fnName: string, params: string[]): IR.IrExpr {
  switch (expr.kind) {
    case "app":
    ...
    case "block": {
      if (expr.exprs.length === 0) return expr;
