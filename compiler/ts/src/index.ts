...
export function parseRunArgs(args: string[]): { flags: Record<string, string>; positional: string[] } {
  return parseCliArgs(args);
}

export function extractNodeConfig(flags: Record<string, string>): DistributedNodeConfig | null {
  const nodeFlag = flags['node'];
  if (!nodeFlag) return null;
...
function runLocal(inputFile: string, strict = false): void {
...
function runWithNode(inputFile: string, nodeConfig: DistributedNodeConfig): void {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'japl-'));
...
function cmdCluster(args: string[]): void {
