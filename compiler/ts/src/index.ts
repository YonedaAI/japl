...
import { buildFile, buildToWat, buildToWasm, runWasm, checkTools, findJaplRuntime, programUsesProcesses } from './cli/build.js';
...
function cmdRun(args: string[]): void {
  ...
  try {
    ...
    if (needsRuntime) {
      ...
      if (flags['node'] !== undefined) runtimeArgs.push('--node', flags['node']);
      if (flags['listen'] !== undefined) runtimeArgs.push('--listen', flags['listen']);
      if (flags['connect'] !== undefined) runtimeArgs.push('--connect', flags['connect']);
