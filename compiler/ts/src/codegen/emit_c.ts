...
const BUILTIN_FNS: Record<string, string> = {
...
export class CEmitter {
  ...
  private functionDecls: string[] = [];  // forward declarations
  private closureCount: number = 0;      // for generating unique closure names
  private lambdaLifts: string[] = [];    // lifted lambda functions
    ...
    for (const decl of module.decls) {
      if (decl.kind === 'fn') {
        this.functionDecls.push(
          `JaplValue japl_fn_${decl.name}(JaplValue* args, int argc, JaplValue* env, int envc);`
        );
      }
    }
  ...
  private emitForwardDecls(): void {
    if (this.functionDecls.length === 0) return;
  ...
  private emitDecl(decl: IR.IrDecl): void {
    switch (decl.kind) {
      case 'fn':
  ...
  private emitTestDecl(decl: IR.IrDecl & { kind: 'test' }): void {
  ...
  private emitExpr(expr: IR.IrExpr): string {
    switch (expr.kind) {
      case 'int':
  ...
  private emitApp(expr: IR.IrExpr & { kind: 'app' }): string {
  ...
  private emitLambda(expr: IR.IrExpr & { kind: 'lambda' }): string {
  ...
  private emitReceive(expr: IR.IrExpr & { kind: 'receive' }): string {
  ...
  private emitMatchAsBlock(expr: IR.IrExpr & { kind: 'match' }): string {
  ...
  private emitLetAsBlock(expr: IR.IrExpr & { kind: 'let' }): string {
  ...
  private emitBlockExpr(expr: IR.IrExpr & { kind: 'block' }): string {
    const parts: string[] = [];
    parts.push('({');
  ...
  private emitExprAsStatements(expr: IR.IrExpr, isReturn: boolean): void {
    switch (expr.kind) {
      case 'let':
  ...
  private emitLetChain(expr: IR.IrExpr & { kind: 'let' }, isReturn: boolean): void {
    this.line(`JaplValue ${this.sanitizeName(expr.name)} = ${this.emitExpr(expr.value)};`);
    this.emitExprAsStatements(expr.body, isReturn);
  }

  private emitIfStatements(expr: IR.IrExpr & { kind: 'if' }, isReturn: boolean): void {
  ...
  private emitPattern(pat: IR.IrPattern, scrutinee: string): { condition: string; bindings: string[] } {
    switch (pat.kind) {
      case 'pvar':
        return {
          condition: 'true',
  ...
  private collectFreeVars(expr: IR.IrExpr, bound: Set<string>): string[] {
    const free = new Set<string>();
    this.collectFreeVarsImpl(expr, bound, free);
    return [...free];
  }

  private collectFreeVarsImpl(expr: IR.IrExpr, bound: Set<string>, free: Set<string>): void {
    switch (expr.kind) {
      case 'var':
      ...
      case 'app':
        ...
        for (const a of expr.args) this.collectFreeVarsImpl(a, bound, free);
        break;
      case 'if':
      ...
      case 'unaryop':
        ...
        break;
      case 'record':
        for (const [, v] of expr.fields) this.collectFreeVarsImpl(v, bound, free);
        break;
      case 'field_access':
      ...
      case 'record_update':
        ...
        for (const [, v] of expr.updates) this.collectFreeVarsImpl(v, bound, free);
        break;
      case 'list':
        for (const e of expr.elements) this.collectFreeVarsImpl(e, bound, free);
        break;
      case 'construct':
        for (const a of expr.args) this.collectFreeVarsImpl(a, bound, free);
        break;
      case 'block':
        for (const e of expr.exprs) this.collectFreeVarsImpl(e, bound, free);
        break;
      case 'spawn':
        this.collectFreeVarsImpl(expr.fn, bound, free);
        break;
      case 'send':
        ...
        break;
      case 'receive':
        for (const arm of expr.arms) this.collectFreeVarsImpl(arm.body, bound, free);
        break;
      case 'try':
        this.collectFreeVarsImpl(expr.expr, bound, free);
        break;
  ...
  private collectPatternBindings(pat: IR.IrPattern, bound: Set<string>): void {
    switch (pat.kind) {
      case 'pvar':
        bound.add(pat.name);
        break;
      case 'pconstructor':
        for (const arg of pat.args) this.collectPatternBindings(arg, bound);
        break;
      case 'plist':
        for (const el of pat.elements) this.collectPatternBindings(el, bound);
  ...
  private isSimpleExpr(expr: IR.IrExpr): boolean {
    switch (expr.kind) {
      case 'int':
      case 'float':
      case 'string':
