...
export class TsEmitter {
  ...
  private scanImports(decl: IR.IrDecl): void {
    switch (decl.kind) {
      case "fn":
  ...
  private scanExprImports(expr: IR.IrExpr): void {
    switch (expr.kind) {
      case "spawn":
      ...
      case "match":
        ...
        for (const arm of expr.arms) this.scanExprImports(arm.body);
        break;
      case "binop":
      ...
      case "unaryop":
        ...
        break;
      case "record":
        for (const [, v] of expr.fields) this.scanExprImports(v);
        break;
      case "field_access":
      ...
      case "record_update":
        ...
        for (const [, v] of expr.updates) this.scanExprImports(v);
        break;
      case "list":
        for (const e of expr.elements) this.scanExprImports(e);
        break;
      case "block":
        for (const e of expr.exprs) this.scanExprImports(e);
        break;
      case "try":
        this.scanExprImports(expr.expr);
        break;
  ...
  private emitRecordTypeDecl(decl: IR.IrDecl & { kind: "record_type" }): void {
  ...
  private emitLetAsIife(expr: IR.IrExpr & { kind: "let" }): string {
    const lines: string[] = [];
    lines.push("(() => {");
    const saved = this.output;
    this.output = [];
  ...
  private emitIfAsIife(expr: IR.IrExpr & { kind: "if" }): string {
    const lines: string[] = [];
    lines.push("(() => {");
    const saved = this.output;
    this.output = [];
  ...
  private emitMatchAsIife(expr: IR.IrExpr & { kind: "match" }): string {
    const lines: string[] = [];
    lines.push("(() => {");
    const saved = this.output;
    this.output = [];
  ...
  private emitBlockAsIife(expr: IR.IrExpr & { kind: "block" }): string {
    const lines: string[] = [];
    lines.push("(() => {");
    const saved = this.output;
    this.output = [];
  ...
  private emitReceiveAsIife(expr: IR.IrExpr & { kind: "receive" }): string {
    const lines: string[] = [];
    lines.push("(async () => {");
    const saved = this.output;
    this.output = [];
  ...
  private emitTry(expr: IR.IrExpr & { kind: "try" }): string {
  ...
  private emitExprAsReturn(expr: IR.IrExpr): string {
    ...
    if (allConstructors) {
      ...
      this.indented(() => {
        for (const arm of arms) {
          const pat = arm.pattern as IR.IrPattern & { kind: "pconstructor" };
  ...
  private emitPatternCondition(pat: IR.IrPattern, scrStr: string): string {
    switch (pat.kind) {
      case "pvar":
        return "true";
      case "pwildcard":
  ...
  private emitPatternBindings(pat: IR.IrPattern, scrStr: string): void {
    switch (pat.kind) {
      case "pvar":
